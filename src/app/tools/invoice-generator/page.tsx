...
export default function InvoiceGeneratorPage() {
  ...
  const handlePrint = async () => {
      ...
      // Header
      page.drawText("INVOICE", { x: margin, y, font: fontBold, size: 28, color: rgb(0.1, 0.1, 0.1) });
      page.drawText(invoiceNumber, { x: width - margin - font.widthOfTextAtSize(invoiceNumber, 12), y, font, size: 12, color: rgb(0.4, 0.4, 0.4) });
      y -= 40;

      // From
      page.drawText("From:", { x: margin, y, font: fontBold, size: 10, color: rgb(0.4, 0.4, 0.4) });
      y -= 16;
      if (from.name) { page.drawText(from.name, { x: margin, y, font: fontBold, size: 11 }); y -= 14; }
      if (from.email) { page.drawText(from.email, { x: margin, y, font, size: 9, color: rgb(0.4, 0.4, 0.4) }); y -= 14; }
      if (from.address) { page.drawText(from.address, { x: margin, y, font, size: 9, color: rgb(0.4, 0.4, 0.4) }); y -= 14; }
      y -= 10;

      // To
      page.drawText("Bill To:", { x: margin, y, font: fontBold, size: 10, color: rgb(0.4, 0.4, 0.4) });
      y -= 16;
      if (to.name) { page.drawText(to.name, { x: margin, y, font: fontBold, size: 11 }); y -= 14; }
      if (to.email) { page.drawText(to.email, { x: margin, y, font, size: 9, color: rgb(0.4, 0.4, 0.4) }); y -= 14; }
      if (to.address) { page.drawText(to.address, { x: margin, y, font, size: 9, color: rgb(0.4, 0.4, 0.4) }); y -= 14; }
      y -= 10;

      // Dates
      page.drawText("Date: " + invoiceDate, { x: width - margin - 150, y: height - 100, font, size: 9, color: rgb(0.4, 0.4, 0.4) });
      if (dueDate) { page.drawText("Due: " + dueDate, { x: width - margin - 150, y: height - 114, font, size: 9, color: rgb(0.4, 0.4, 0.4) }); }

      // Table header
      y -= 20;
      page.drawRectangle({ x: margin, y: y - 4, width: width - margin * 2, height: 22, color: rgb(0.95, 0.95, 0.95) });
      page.drawText("Description", { x: margin + 8, y, font: fontBold, size: 9 });
      page.drawText("Qty", { x: 340, y, font: fontBold, size: 9 });
      page.drawText("Price", { x: 400, y, font: fontBold, size: 9 });
      ...
      for (const item of validItems) {
        ...
        page.drawText(item.description.slice(0, 45), { x: margin + 8, y, font, size: 9 });
        page.drawText(String(item.quantity), { x: 340, y, font, size: 9 });
        page.drawText(currency + item.price.toFixed(2), { x: 400, y, font, size: 9 });
        page.drawText(currency + lineTotal.toFixed(2), { x: 480, y, font, size: 9 });
        y -= 18;
      }

      // Totals
      y -= 10;
      page.drawLine({ start: { x: 380, y: y + 8 }, end: { x: width - margin, y: y + 8 }, thickness: 0.5, color: rgb(0.8, 0.8, 0.8) });
      const subtotalVal = validItems.reduce((sum, item) => sum + item.quantity * item.price, 0);
      page.drawText("Subtotal:", { x: 400, y, font, size: 9 });
      page.drawText(currency + subtotalVal.toFixed(2), { x: 480, y, font, size: 9 });
      y -= 16;
      if (taxRate > 0) {
        const taxVal = subtotalVal * (taxRate / 100);
        page.drawText("Tax (" + taxRate + "%):", { x: 400, y, font, size: 9 });
        page.drawText(currency + taxVal.toFixed(2), { x: 480, y, font, size: 9 });
        y -= 16;
      }
      const totalVal = subtotalVal * (1 + taxRate / 100);
      page.drawText("Total:", { x: 400, y, font: fontBold, size: 11 });
      page.drawText(currency + totalVal.toFixed(2), { x: 480, y, font: fontBold, size: 11 });

      // Notes
      if (notes) {
        y -= 40;
        page.drawText("Notes:", { x: margin, y, font: fontBold, size: 9, color: rgb(0.4, 0.4, 0.4) });
        y -= 14;
        page.drawText(notes.slice(0, 200), { x: margin, y, font, size: 8, color: rgb(0.5, 0.5, 0.5) });
      }
